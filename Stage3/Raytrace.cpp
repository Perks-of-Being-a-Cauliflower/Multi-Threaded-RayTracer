...
void render(Scene* scene, const int width, const int height, const int aaLevel, bool testMode, unsigned int threadID, int numThreads, LONG* sharedMem, unsigned int blockSize, bool colourise)
{
	...
	// current block being rendered counter
	unsigned int curBlock = 0;
	// count of samples rendered
	unsigned int samplesRendered = 0;
	// the number of blocks to be rendered
	unsigned int numblocks = ((width * height) / (blockSize* blockSize));


	// get the number of blocks high and wide
	unsigned int numblocksH = height / blockSize;
	unsigned int numblocksW = width / blockSize;

	// if the blocksize does not divide evenly into the width and/or the height then increase the number of blocks need by one
	...
	// get the new number of blocks needed
	numblocks = numblocksH * numblocksW;

	// run loop while the number of blocks done is less that the total number of blocks
	while ((curBlock = InterlockedIncrement(sharedMem)) < numblocks) {

		// set the initial offset to just be the block size
		int blockOffsetX = blockSize;
		int blockOffsetY = blockSize;

		// check if the width and height are divisable by the block size
		if (((width * height) % (blockSize * blockSize)) != 0) {
			
			// check if its the last block in the row to render
			if (((curBlock) % (numblocksW)) == (numblocksW - 1)) {
				// make the x offset equal to the difference
				blockOffsetX = width % blockSize; 


			}
			// check if its the last block in the column to render
			if (curBlock >= (numblocks - numblocksW)) {
				// make the y offset equal to the difference
				blockOffsetY = height % blockSize; 
			}
		}

		// get the x coordinate based on the current block
		int blockXCo = ((-width / 2) + ((curBlock % numblocksW) * blockSize));
		// get the y coordinate based on the current block
		int blockYCo = ((-height / 2) + ((curBlock / numblocksW) % numblocksH) * blockSize);


		// loop through all the pixels using the offset to set the upper bounds of the loop
		for (int y = blockYCo; y < (blockYCo + blockOffsetY); ++y)
		{
			for (int x = blockXCo; x < (blockXCo + blockOffsetX); ++x)
			{
				...
				else
				{
					...
					out[((y + (height / 2)) * width + (x + (width / 2)))] = Colour(1, 1, 1).convertToPixel(threadID);
				}
			}
		}
	}
...
DWORD __stdcall THREAD_START_ROUTINE(void* threadid)
{
	...
	render(data->scene, data->width, data->height, data->aaLevel, data->testMode, data->threadID, data->numThreads, data->sharedMem, data->blockSize, data->colourise);	// raytrace scene
